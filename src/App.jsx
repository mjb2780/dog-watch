...
// ---------------------------------------------------------------------------
// npoint.io — free shared JSON storage, no account or API key needed
// 1. Go to https://www.npoint.io
// 2. Click "New Bin" — paste: { "inside": true, "updatedAt": "" }
...
async function fetchStatus() {
  ...
  return data // { inside: bool, updatedAt: string }
}

async function saveStatus(inside) {
  const body = JSON.stringify({ inside, updatedAt: new Date().toISOString() })
  const res  = await fetch(BASE_URL, {
    method: 'POST',
    ...
    <div style={root(inside)}>
      ...
      <div style={card}>
        ...
        <button onClick={toggle} disabled={loading || toggling} style={btn(inside, toggling)}>
          ...
          <span style={btnLabel}>
            {loading ? 'Loading…' : inside ? 'INSIDE' : 'OUTSIDE'}
          </span>
          <span style={btnSub}>
            {inside ? 'safe to open the gate' : 'keep the gate closed'}
          </span>
        </button>
        ...
        <div style={statusRow}>
          ...
          <span style={statusText}>
            Dogs are currently{' '}
            <strong>{loading ? '…' : inside ? 'inside' : 'outside'}</strong>
          </span>
        </div>
        ...
        {error && (
          <p style={errorText}>⚠️ Can't reach server — check your connection</p>
        )}
...
const root = (inside) => ({
  ...
  background: inside
    ? 'linear-gradient(135deg,#1a3a2a 0%,#0d2318 60%,#0a1a10 100%)'
    : 'linear-gradient(135deg,#3a2800 0%,#5c3d00 50%,#2a1a00 100%)',
...
const titleStyle = {
  fontSize: '32px', fontWeight: '700', color: '#f5f0e8',
  margin: 0, letterSpacing: '0.05em',
}
const subStyle = {
  color: 'rgba(245,240,232,0.45)', fontSize: '13px',
  marginBottom: '40px', letterSpacing: '0.03em',
}

const btn = (inside, toggling) => ({
  width: '100%',
  padding: '32px 24px',
  borderRadius: '18px',
  border: 'none',
  ...
  boxShadow: inside
    ? '0 8px 32px rgba(45,106,79,0.5),inset 0 1px 0 rgba(255,255,255,0.1)'
    : '0 8px 32px rgba(180,83,9,0.5),inset 0 1px 0 rgba(255,255,255,0.1)',
  display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px',
  transition: 'all 0.3s ease',
  transform: toggling ? 'scale(0.97)' : 'scale(1)',
  marginBottom: '28px',
...
const rippleAnim = (inside) => ({
  position: 'absolute',
  top: '50%', left: '50%',
  width: '400px', height: '400px',
  marginLeft: '-200px', marginTop: '-200px',
  borderRadius: '50%',
  background: inside ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.12)',
  animation: 'ripple 0.7s ease-out forwards',
})

const btnIcon  = { fontSize: '52px', display: 'block', lineHeight: 1 }
const btnLabel = {
  fontSize: '28px', fontWeight: '800', color: '#fff', letterSpacing: '0.12em',
}
const btnSub = {
...
const dot = (inside, loading) => ({
  width: '8px', height: '8px', borderRadius: '50%', flexShrink: 0,
  background: loading ? '#888' : inside ? '#4ade80' : '#fb923c',
  boxShadow: loading ? 'none' : inside ? '0 0 8px #4ade80' : '0 0 8px #fb923c',
  animation: loading ? 'pulse 1.2s ease-in-out infinite' : 'none',
})

const statusText = { color: 'rgba(245,240,232,0.7)', fontSize: '15px' }
const timeText   = {
  color: 'rgba(245,240,232,0.35)', fontSize: '12px',
...
const hintText   = {
  color: 'rgba(245,240,232,0.22)', fontSize: '11px',
  letterSpacing: '0.04em', textTransform: 'uppercase', marginTop: '8px',
}
